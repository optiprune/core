  ...
  for (const module of context.modules.values()) {
    ...
    walkAst(module.ast, (rawNode) => {
      ...
      if (decorators.length > 0) {
        for (const decorator of decorators) {
          ...
          if (PROTECTED_DECORATORS.has(decoratorName)) {
            ...
            if (
              (targetNode.type === "ClassDeclaration" ||
                targetNode.type === "FunctionDeclaration") &&
              targetNode.id?.name
            ) {
              externallyDefinedContracts.add(targetNode.id.name);
            } else if (
              (targetNode.type === "ClassMethod" || targetNode.type === "ClassProperty") &&
              targetNode.key?.type === "Identifier"
            ) {
      ...
      if (Array.isArray(node.leadingComments)) {
        const isPublic = (node.leadingComments as any[]).some(
          (comment: any) =>
