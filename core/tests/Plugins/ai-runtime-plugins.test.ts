...
describe("AI runtime plugins", () => {
  ...
  it("reports ONNX sessions and tensors that are not released and accepts finally-based session release", async () => {
    ...
    expect(findingRules(unsafe)).toEqual(
      expect.arrayContaining(["onnx-unreleased-session", "onnx-unreleased-tensor"]),
    );
  ...
  it("reports Transformers.js pipeline creation only in a loop or request handler", async () => {
    ...
    expect(
      report.findings.filter((finding) => finding.rule === "transformers-pipeline-hot-path"),
    ).toHaveLength(2);
  });
  ...
  it("reports unbounded Vercel AI tool loops and request streams lacking onError", async () => {
    ...
    expect(findingRules(report)).toEqual(
      expect.arrayContaining(["ai-sdk-unbounded-tool-loop", "ai-sdk-stream-no-error-handler"]),
    );
  });
  ...
  it("reports direct user-looking prompt interpolation and graph invocations without recursionLimit", async () => {
    const report = await inspect(
      { "@langchain/core": "^0.0.0", "@langchain/langgraph": "^0.0.0" },
      `
    ...
    expect(findingRules(report)).toEqual(
      expect.arrayContaining([
        "langchain-direct-prompt-interpolation",
        "langgraph-missing-recursion-limit",
      ]),
    );
  });
