...
describe("new analysis options", () => {
  it("does not report exports in an entry file unless includeEntryExports is enabled", async () => {
    ...
    expect(
      normal.findings.some(
        (f) => f.rule === "unused-export" && f.evidence.exportName === "deadValue",
      ),
    ).toBe(false);
    ...
    expect(
      result.findings.some(
        (finding) =>
          ...
          finding.evidence.memberName === "onlyLoadedDynamically",
      ),
    ).toBe(false);
  });
  ...
  it("ignores test files when ignoreTests is enabled", async () => {
    const root = await rootWith({
      ...
      "tests/fixtures/ignored-package/optiprune.config.ts":
        "throw new Error('test config must not be loaded');\\n",
    });
    const normal = await analyze({
      rootDir: root,
      entry: ["src/index.ts"],
    ...
    expect(warningSpy).not.toHaveBeenCalledWith(
      expect.stringContaining("test config must not be loaded"),
    );
...
describe("export fixer syntax", () => {
  it.each([".js", ".jsx", ".ts", ".tsx", ".vue"])("supports %s", async (extension) => {
    ...
    expect(
      await applyFixes(report([unusedExport(file, "dead")]), root, {
        rules: ["exports"],
        confidence: "high",
      }),
    ).toBe(1);
  ...
  it("removes only export when a symbol is used in the same file", async () => {
    ...
    expect(
      await applyFixes(report([unusedExport(file, "used")]), root, { rules: ["exports"] }),
    ).toBe(1);
    expect(await fs.readFile(path.join(root, file), "utf8")).toBe(
      "const used = 1;\nconsole.log(used);\n",
    );
  ...
  it("removes a middle export-list item and its preceding comma", async () => {
    ...
    expect(
      await applyFixes(report([unusedExport(file, "two")]), root, { rules: ["exports"] }),
    ).toBe(1);
