...
describe("applyFixes", () => {
  it("removes unused dependencies only when dependencies is selected", async () => {
    const root = await fixture({
      ...
      devDependencies: { devUnused: "1.0.0" },
    });
  ...
  it("removes devDependencies only when devDependencies is selected", async () => {
    ...
    const findings = report([
      finding("unused-dev-dependency", "high", "package.json", { package: "remove" }),
    ]);
  ...
  it("removes separately declared exports from multiline lists without corrupting syntax", async () => {
    ...
    expect(await applyFixes(report([unusedCard]), root, { rules: ["exports"], force: true })).toBe(
      1,
    );
  ...
  it("removes an unused exported alias while preserving the local declaration", async () => {
    ...
    expect(await applyFixes(report([unusedAlias]), root, { rules: ["exports"], force: true })).toBe(
      1,
    );
  ...
  it("honors confidence thresholds independently from force", async () => {
    ...
    expect(
      await applyFixes(report([lowFinding]), root, { rules: ["exports"], confidence: "high" }),
    ).toBe(0);
    expect(await fs.readFile(path.join(root, "src.ts"), "utf8")).toContain("export const");
    expect(
      await applyFixes(report([lowFinding]), root, {
        rules: ["exports"],
        confidence: "high",
        force: true,
      }),
...
describe("fixer regression fixtures", () => {
  async function copyFixture(): Promise<string> {
    ...
    temporaryRoots.push(root);
    await fs.cp(path.resolve("core/tests/fixtures/functions/fixer"), root, { recursive: true });
    return root;
  }
