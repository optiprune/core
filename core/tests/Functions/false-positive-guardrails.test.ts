...
describe("false-positive guardrails", () => {
  it("resolves project-local package.json imports aliases as source modules", async () => {
    ...
    expect(
      isFinding(report, "missing-dependency", (finding) => finding.evidence.package === "#config"),
    ).toBe(false);
    expect(
      isFinding(report, "unreachable-file", (finding) => finding.file.endsWith("src/config.js")),
    ).toBe(false);
  });
  ...
  it("treats require.resolve as concrete dependency usage", async () => {
    ...
    expect(
      isFinding(
        report,
        "unused-dependency",
        (finding) => finding.evidence.package === "fixture-tool",
      ),
    ).toBe(false);
  });
  ...
  it("uses TypeScript runner targets and local Node preloads as runtime entry points", async () => {
    ...
    for (const suffix of ["scripts/seed.tsx", "scripts/register.mjs", "scripts/main.mjs"]) {
      expect(
        isFinding(report, "unreachable-file", (finding) => finding.file.endsWith(suffix)),
      ).toBe(false);
    }
  });
  ...
  it("reports dependencies used only by unreachable files, but leaves them installed when files are retained", async () => {
    ...
    expect(
      isFinding(
        report,
        "missing-dependency",
        (finding) => finding.evidence.package === "missing-only-dead",
      ),
    ).toBe(false);
  ...
  it("removes conditionally unused dependencies only with their supporting unreachable file or after it is absent", async () => {
    ...
    expect(
      await fs.access(path.join(root, "src", "dead.ts")).then(
        () => true,
        () => false,
      ),
    ).toBe(false);
    ...
    const alreadyRemovedReport = await analyze({
      rootDir: alreadyRemovedRoot,
      extensions: [".ts"],
      layers: { skip3: true, skip4: true },
    });
    await fs.rm(path.join(alreadyRemovedRoot, "src", "dead.ts"));
    expect(
      await applyFixes(alreadyRemovedReport, alreadyRemovedRoot, {
        rules: ["dependencies", "devDependencies"],
        confidence: "medium+",
      }),
    ).toBe(2);
    const afterPriorRemoval = JSON.parse(
      await fs.readFile(path.join(alreadyRemovedRoot, "package.json"), "utf8"),
    );
