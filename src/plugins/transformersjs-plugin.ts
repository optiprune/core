...
function stateFor(file: string): FileState {
  ...
  if (!state) {
    state = { pipelineNames: new Set(["pipeline"]), namespaceNames: new Set(["transformers"]), emitted: new Set() };
...
function recordImport(node: any, state: FileState, adapter: PluginAdapter): void {
  ...
  for (const specifier of node.specifiers ?? []) {
    ...
    if (["ImportNamespaceSpecifier", "ImportDefaultSpecifier"].includes(specifier.type) && local) state.namespaceNames.add(local);
  }
}
...
function isPipelineInstantiation(node: any, state: FileState): boolean {
  ...
  return !!member && member.method === "pipeline" && t.isIdentifier(member.object) && state.namespaceNames.has(member.object.name);
}
  ...
  lifecycle: {
    ...
    onASTNode: (node: any, file, adapter, ancestors = []) => {
      ...
      emitOnce(state.emitted, adapter, `pipeline-hot-path:${file}:${node.start}`, {
        ...
        confidence: "high",
        file,
        message: "Transformers.js pipeline() is instantiated in a loop or request handler. Cache the pipeline at module or service scope to avoid repeated model loading, latency, and memory pressure.",
        evidence: { inLoop, inRequestHandler: inHandler, recommendation: "cache pipeline() outside the hot path" },
      });
    },
  },
};
