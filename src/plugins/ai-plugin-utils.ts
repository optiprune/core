...
export function hasRequestHandlerAncestor(ancestors: any[]): boolean {
  ...
  if (
    fn?.params?.some((parameter: any) =>
      ["req", "res", "request", "response", "reply", "ctx", "event"].includes(
        nodeName(parameter) ?? "",
      ),
    )
  ) {
  ...
  return ancestors.some((node) => {
    ...
    return (
      !!member &&
      ["get", "post", "put", "patch", "delete", "all", "use", "route", "handle", "on"].includes(
        member.method,
      )
    );
  });
...
export function isWithinTidy(node: any, ancestors: any[], tfNames: Set<string>): boolean {
  return ancestors.some((ancestor) => {
    ...
    return (
      ...
      tfNames.has(member.object.name) &&
      rangeContains(ancestor, node)
    );
  });
}
...
export function isPackageRequire(node: any, packageNames: Set<string>): boolean {
  return (
    ...
    packageNames.has(nodeName(node.arguments?.[0]) ?? "")
  );
}

export async function hasDeclaredPackage(
