...
function stateFor(file: string): FileState {
  ...
  if (!state) {
    state = {
      callNames: new Map([["generateText", "generateText"], ["streamText", "streamText"], ["generateObject", "generateObject"], ["streamObject", "streamObject"]]),
...
function recordImport(node: any, state: FileState, adapter: PluginAdapter): void {
  ...
  for (const specifier of node.specifiers ?? []) {
    ...
    if (["ImportNamespaceSpecifier", "ImportDefaultSpecifier"].includes(specifier.type)) state.namespaceNames.add(local);
  }
}
...
function callKind(node: any, state: FileState): string | undefined {
  ...
  return member && t.isIdentifier(member.object) && state.namespaceNames.has(member.object.name) && TOOL_CALLS.has(member.method) ? member.method : undefined;
}
...
function auditOptions(kind: string, options: any, node: any, file: string, ancestors: any[], state: FileState, adapter: PluginAdapter): void {
  ...
  if (kind === "streamText" && hasRequestHandlerAncestor(ancestors) && !hasObjectProperty(options, "onError")) {
    emitOnce(state.emitted, adapter, `stream-no-error-handler:${file}:${node.start}`, {
      rule: "ai-sdk-stream-no-error-handler",
      severity: "warning",
      confidence: "medium",
      file,
      message: "streamText() is returned from a request handler without an onError callback. Provide stream error handling so failures are surfaced safely to the caller and logs.",
...
function auditAgent(node: any, file: string, state: FileState, adapter: PluginAdapter): void {
  ...
  if (!t.isNewExpression(expression) || !t.isIdentifier(expression.callee) || !state.agentNames.has(expression.callee.name)) return;
  const options = expression.arguments?.[0];
  if (!t.isObjectExpression(options) || !hasObjectProperty(options, "tools") || hasBound(options)) return;
  emitOnce(state.emitted, adapter, `unbounded-agent:${file}:${node.start}`, {
    rule: "ai-sdk-unbounded-tool-loop",
    severity: "warning",
    confidence: "high",
    file,
    message: "ToolLoopAgent is configured with tools but without a static maxSteps/stopWhen bound. Add an explicit finite loop limit.",
