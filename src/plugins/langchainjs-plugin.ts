...
function graphFor(state: FileState, name: string, scope: string): GraphResource | undefined {
  return state.compiledGraphs.get(keyFor(scope, name)) ?? state.compiledGraphs.get(keyFor("module", name));
}
...
function isPromptTemplateCall(node: any, state: FileState): boolean {
  ...
  return !!call && call.method === "fromTemplate" && t.isIdentifier(call.object) && state.promptClasses.has(call.object.name);
}
...
function containsUserControlledValue(node: any): boolean {
  ...
  if (node.type === "MemberExpression" || node.type === "OptionalMemberExpression") return containsUserControlledValue(node.object) || containsUserControlledValue(node.property);
  if (node.type === "TemplateLiteral") return (node.expressions ?? []).some(containsUserControlledValue);
  if (node.type === "BinaryExpression") return containsUserControlledValue(node.left) || containsUserControlledValue(node.right);
  return false;
}
...
function isPromptConstructionAncestor(ancestors: any[], state: FileState): boolean {
  return ancestors.some((ancestor) => {
    ...
    return t.isNewExpression(expression) && t.isIdentifier(expression.callee) && state.promptClasses.has(expression.callee.name);
  });
}

function auditPrompt(node: any, file: string, ancestors: any[], state: FileState, adapter: PluginAdapter): void {
  ...
  if ((!isTemplateLiteral && !isConcatenation) || !containsUserControlledValue(node) || !isPromptConstructionAncestor(ancestors, state)) return;
  emitOnce(state.emitted, adapter, `prompt-injection:${file}:${node.start}`, {
    rule: "langchain-direct-prompt-interpolation",
    severity: "warning",
    confidence: "medium",
    file,
    message: "LangChain prompt template directly interpolates a value with a user-controlled-looking name. Prefer structured message variables and validate or delimit untrusted content to reduce prompt-injection risk.",
    evidence: { pattern: isTemplateLiteral ? "template-literal" : "string-concatenation", recommendation: "structured prompt variables" },
  });
}
...
function recordGraph(node: any, file: string, ancestors: any[], state: FileState): void {
  ...
  if (t.isNewExpression(init) && t.isIdentifier(init.callee) && state.stateGraphClasses.has(init.callee.name)) {
  ...
  if (compile?.method === "compile" && t.isIdentifier(compile.object)) {
    ...
    if (!state.graphBuilders.has(builderKey) && !state.graphBuilders.has(keyFor("module", compile.object.name))) return;
    const graph: GraphResource = { key: keyFor(scope, node.id.name), name: node.id.name, file, scope };
    state.compiledGraphs.set(graph.key, graph);
  }
}

function auditGraphInvocation(node: any, file: string, ancestors: any[], state: FileState, adapter: PluginAdapter): void {
  if (!t.isCallExpression(node)) return;
  const call = getMemberCall(node);
  if (!call || !["invoke", "stream", "batch"].includes(call.method) || !t.isIdentifier(call.object)) return;
