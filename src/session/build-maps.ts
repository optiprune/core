...
export const buildExportsMap = (
  ...
  const addExport = (
    ...
    for (const location of usage.locations)
      if (location.isEntry) usageEntryPaths.add(location.filePath);
  ...
  for (const [importedFilePath, importMaps] of fileNode.imports.internal) {
    ...
    forEachNamespaceReExport(importMaps, (namespace, _sources) => {
      ...
      const namespaceImport =
        getImport(importedFilePath, namespace) ?? getImport(importedFilePath, IMPORT_STAR);
...
export const buildInternalImports = (
  fileNode: FileNode,
  explorer: GraphExplorer,
  importLookup: ImportLookup,
) => {
