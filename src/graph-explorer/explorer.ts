...
export const createGraphExplorer = (graph: ModuleGraph, entryPaths: Set<string>) => {
  return {
    ...
    resolveDefinition: (filePath: string, identifier: string) =>
      resolveDefinition(graph, filePath, identifier),
    getUsage: (filePath: string, identifier: string) =>
      getUsage(graph, entryPaths, filePath, identifier),
