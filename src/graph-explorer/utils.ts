...
import type { FileNode, Import, ImportMaps, ModuleGraph } from "../types/module-graph.js";
...
export const getRuntimeSuccessors = (
  node: FileNode,
  ignoredFlags = IGNORED_CYCLE_IMPORT_FLAGS,
): Set<string> => {
...
export const getRuntimeImport = (
  ...
  for (const _import of node.imports.imports) {
    ...
    if (
      !result ||
      _import.line < result.line ||
      (_import.line === result.line && _import.col < result.col)
    ) {
      result = _import;
    }
  }
...
export const hasStrictlyEnumReferences = (
