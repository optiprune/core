...
import type { Identifier, ImportMaps, ModuleGraph } from "../../types/module-graph.js";
import {
  getAliasReExportMap,
...
export const isReferenced = (
  ...
  const hasCompleteResult = () =>
    isReferenced && (options.traverseEntries || reExportingEntryFile !== undefined);
  ...
  const walkDown = (path: string, id: string, viaStar = false): boolean => {
    ...
    if (!isReferenced) {
      ...
      const hasDirectReference =
        ((file.import.get(OPAQUE) || (isLoaderImport && !node.exports.has("default"))) &&
          !hasOnlyNsRefs(file)) ||
