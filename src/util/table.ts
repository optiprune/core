...
export class Table {
  ...
  cell(column: string, value: Value, formatter?: (value: Value) => string) {
    ...
    if (!row) throw new Error("A row must be created before adding cells");
  ...
  private distributeWidths(
    ...
    const reserved = columns
      .filter((col) => this.modeFor(col) === "none")
      .reduce((sum, col) => sum + (widths[col] ?? 0), 0);

    const budget = Math.max(0, this.maxWidth - separatorWidth - reserved);
    const original: Record<string, number> = {};
    for (const col of truncatable) original[col] = widths[col] ?? 0;
    ...
    while (changed && unresolved.size > 0) {
      ...
      for (const col of unresolved) {
        if ((original[col] ?? 0) <= share) {
          widths[col] = original[col] ?? 0;
          remainingBudget -= original[col] ?? 0;
    ...
    const overMin = [...unresolved].reduce(
      (sum, col) => sum + ((original[col] ?? 0) - MIN_TRUNCATED_WIDTH),
      0,
    );
    ...
    for (const col of unresolved) {
      const share =
        overMin > 0
          ? Math.floor((((original[col] ?? 0) - MIN_TRUNCATED_WIDTH) * excess) / overMin)
          : 0;
    ...
    if (leftover > 0) {
      ...
      if (first) widths[first] = (widths[first] ?? 0) + leftover;
    }
  }

  toCells() {
    const columns = this.columns.filter((col) =>
      this.rows.some((row) => isPrintable(row[col]?.value)),
    );
    ...
    const totalWidth =
      columns.reduce((sum, col) => sum + (columnWidths[col] ?? 0), 0) + separatorWidth;
    ...
    return rows.map((row) =>
      columns.map((col, index) => {
        ...
        const padded = pad(display, width ?? 0, fill, cell?.align);
        const mode = this.modeFor(col);
        const rendered =
          mode === "none"
            ? padded
            : mode === "start"
              ? truncateStart(padded, width ?? 0)
              : truncate(padded, width ?? 0);
