...
const IS_DTS = /\.d\.(?:ts|mts|cts)$/;
type PackageJson = Record<string, any>;

function getPackageNameFromModuleSpecifier(specifier: string): string | undefined {
...
const isLocalTypeTarget = (target: string) =>
  !target.startsWith("/") &&
  !target.split("/").some((segment) => segment === ".." || segment === "node_modules");
...
const collectPublishedTypeTargets = (value: unknown, targets: Set<string>) => {
  ...
  const typeConditions = children.filter(
    ([condition]) => condition === "types" || condition.startsWith("types@"),
  );
...
export const getPublishedTypeEntrySpecifiers = (manifest: PackageJson) => {
  ...
  if (manifest.typesVersions) {
    for (const paths of Object.values(
      manifest.typesVersions as Record<string, Record<string, unknown>>,
    )) {
...
export const getPackageMapTarget = (map: unknown, key: string) => {
  ...
  let best:
    | { target: unknown; patternMatch: string; prefixLength: number; subpathLength: number }
    | undefined;
  for (const [subpath, target] of entries) {
...
const matchPublishedTypeTarget = (pattern: string, candidate: string) => {
  ...
  const patternMatch = candidate.slice(
    parts[0]?.length ?? 0,
    (parts[0]?.length ?? 0) + matchLength,
  );
...
export const isPublishedTypeExportTarget = (exports: PackageJson["exports"], candidate: string) => {
  ...
  for (const [subpath, target] of getExportEntries(exports)) {
    for (const pattern of getPublishedTypeExportSpecifiers(target)) {
      ...
      if (!match) continue;
      const publicSubpath =
        match.patternMatch === undefined ? subpath : subpath.replaceAll("*", match.patternMatch);
...
export const getEntrySpecifiersFromManifest = (manifest: PackageJson) => {
  ...
  if (bin && typeof bin === "object")
    for (const id of Object.values(bin)) if (typeof id === "string" && id) entryPaths.add(id);
  ...
  if (manifest.imports) {
    for (const [key, value] of Object.entries(manifest.imports)) {
      ...
      for (const item of getEntriesFromExports(value as any)) {
...
export const getManifestImportDependencies = (manifest: PackageJson) => {
  ...
  for (const [entry, exportValue] of Object.entries(manifest.imports)) {
    ...
    for (const item of getEntriesFromExports(exportValue as any)) {
