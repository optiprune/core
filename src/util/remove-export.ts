...
export const removeExport = ({ text, start, end, flags }: FixerOptions) => {
  ...
  if (flags & FIX_FLAGS.EMPTY_DECLARATION && closingBracketOffset !== -1) {
    ...
    if (closingBracketOffset !== -1 && openingBracketIndex !== -1) {
      ...
      const exportLength = beforeBracket.endsWith("export")
        ? 6
        : beforeBracket.endsWith("export type")
          ? 11
          : 0;
      ...
      if (exportLength) {
        ...
        if (fromBracket.startsWith("from")) {
          ...
          if (specifierQuoteMatch?.index) {
            ...
            return (
              beforeBracket.substring(0, exportKeywordOffset) +
              afterEnd.substring(fromSpecifierLength)
            );
          }
        }
        return (
          beforeBracket.substring(0, exportKeywordOffset) + afterEnd.substring(closingBracketOffset)
        );
      }
    }
  }
  ...
  return (
    beforeStart + (removeAfterLength === -1 ? afterEnd : afterEnd.substring(removeAfterLength))
  );
};
