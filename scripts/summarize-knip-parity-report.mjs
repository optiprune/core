...
for (const file of report.testResults ?? []) {
  for (const assertion of file.assertionResults ?? []) {
    ...
    rows.push({ fixture, kind: detail?.[1] ?? "unknown", file: detail?.[2] ?? "", value: detail?.[3] ?? "", message: message.split("\n")[0] });
  }
}
fs.writeFileSync("/tmp/knip-parity-failures.tsv", rows.map((row) => Object.values(row).join("\t")).join("\n") + "\n");
