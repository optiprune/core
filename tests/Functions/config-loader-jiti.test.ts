...
afterEach(async () => {
  await Promise.all(
    temporaryRoots.splice(0).map((rootDir) => fs.rm(rootDir, { recursive: true, force: true })),
  );
});
...
describe("Jiti configuration loading", () => {
  ...
  it("ignores every file below test-like directories, including package.json", async () => {
    ...
    await write(
      rootDir,
      "tests/nested/package.json",
      JSON.stringify({ dependencies: { "unused-test-dependency": "1.0.0" } }),
    );
    ...
    await write(
      rootDir,
      "optiprune.config.ts",
      "export default { entry: ['src/index.ts'], ignoreTests: true, failOn: 'none' };\n",
    );
    ...
    expect(
      report.findings.some((finding) =>
        /(^|[/\\\\])(test|tests|spec|specs|fixtures|__tests__)([/\\\\]|$)|(?:^|[.])(test|spec)[.]/i.test(
          path.relative(rootDir, String(finding.file)),
        ),
      ),
    ).toBe(false);
    expect(
  ...
  it("loads knip.ts as a compatibility configuration", async () => {
