...
async function createFixture(): Promise<string> {
  ...
  await fs.writeFile(path.join(root, "src", "index.ts"), "import './orphan.ts';\nimport './script.ts';\nexport const entry = true;\n");
  await fs.writeFile(
...
describe("whole-file unused export reporting", () => {
  it("reports a pure export-only file as unreachable but preserves files with runtime logic", async () => {
    ...
    try {
      ...
      expect(report.findings.some((finding) =>
        finding.rule === "unused-export" && finding.file.endsWith("src/orphan.ts") && finding.evidence?.exportName === "module1",
      )).toBe(true);
      expect(report.findings.some((finding) =>
        finding.rule === "unreachable-file" && finding.file.endsWith("src/script.ts"),
      )).toBe(false);
      expect(report.findings.some((finding) =>
        finding.rule === "unused-export" && finding.file.endsWith("src/script.ts") && finding.evidence?.exportName === "module2",
      )).toBe(true);
    } finally {
