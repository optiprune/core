...
describe("Dynamic Scan & Import Heuristic (Fix 2)", () => {
  it("should mark files in scanned directories as maybe-reachable when dynamic imports are present", async () => {
    ...
    const loaderUnreachable = report.findings.find(
      (f) => f.file.includes("loader.ts") && f.rule === "unreachable-file",
    );
    ...
    const pluginAUnreachable = report.findings.find(
      (f) => f.file.includes("plugin-a.ts") && f.rule === "unreachable-file",
    );
    const pluginBUnreachable = report.findings.find(
      (f) => f.file.includes("plugin-b.ts") && f.rule === "unreachable-file",
    );
