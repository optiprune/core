...
describe("Optiprune Maintenance Fixes", () => {
  ...
  it("should isolate parse errors and find unrelated unused exports", async () => {
    ...
    const unusedInEntry = report.findings.find(
      (f) => f.rule === "unused-export" && f.evidence.exportName === "unusedInEntry",
    );
  ...
  it("should resolve imports via baseUrl and paths in tsconfig", async () => {
    ...
    const unusedTool = report.findings.find(
      (f) => f.rule === "unused-export" && f.evidence.exportName === "unusedTool",
    );
