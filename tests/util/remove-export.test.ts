...
import { FIX_FLAGS } from "../../src/util/remove-export.js";
import { removeExport } from "../../src/util/remove-export.js";

const getOpts = (
  text: string,
  value: string,
  flags = FIX_FLAGS.OBJECT_BINDING | FIX_FLAGS.EMPTY_DECLARATION,
) => {
...
test("Clean export (FIX_FLAGS.OBJECT_BINDING) - destructuring", () => {
...

  {
    ...
    assert.deepEqual(
      removeExport(getOpts(text, "AB: A_B", 1)),
      "export const {  CD: C_D } = fn();",
    );
  }
});
...
test("Clean export (FIX_FLAGS.WITH_NEWLINE)", () => {
  ...
  }
  {
    const text = "export enum E { AB = 1,\n CD = 2,\n EF = 3 }";
    assert.deepEqual(
      removeExport(getOpts(text, "CD = 2", 5)),
      "export enum E { AB = 1,\n EF = 3 }",
    );
  }
  {
  ...
  }
  {
    const text = "export enum E {\tAB = 1,\n\tCD = 2,\n\tEF = 3,\n}";
    assert.deepEqual(
      removeExport(getOpts(text, "EF = 3", 5)),
      "export enum E {\tAB = 1,\n\tCD = 2,\n\t}",
    );
  }
  {
