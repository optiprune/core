...
describe("Layer 7: Non-Standard Entry & Implicit Binding Engine", () => {
  ...
  it("should detect orphaned event consumers", async () => {
    ...
    const modules = new Map<string, ModuleRecord>();
    modules.set(
      "/test/OrderSubscriber.ts",
      parseModule(consumerSource, "/test/OrderSubscriber.ts"),
    );
    ...
    const orphanedFinding = findings.find(
      (f) => f.rule === "protected-contract" && f.evidence.topic === "order.cancelled",
    );
