...
describe("Layer 6: Dependency & Boundary Engine", () => {
  it("should identify unused dependencies from package.json", async () => {
    ...
    // Mock package.json
    fs.writeFileSync(
      path.join(mockRootDir, "package.json"),
      JSON.stringify({
        dependencies: {
          "used-pkg": "1.0.0",
          "unused-pkg": "2.0.0",
    ...
    fs.writeFileSync(
      path.join(mockRootDir, "package-lock.json"),
      JSON.stringify({
        packages: {
    ...
    const mockContext: Partial<AnalysisContext> = {
      ...
      modules: new Map([
        [
          ...
          {
            id: "file1.ts",
            edges: [{ resolution: "external", rawSpecifier: "used-pkg" }],
          } as any,
        ],
      ]),
    ...
    expect(findings).toContainEqual(
      expect.objectContaining({
        ...
        message: expect.stringContaining(
          "Package 'unused-pkg' is declared as a dependency in package.json but never imported or used in scripts.",
        ),
      }),
    );
  ...
  it("should revoke Layer 5 protection for unreachable files", async () => {
      ...
      modules: new Map([
        [
          "dead.ts",
          {
            ...
            exports: [
              {
                exportedAs: "DeadController",
                isExternalContract: true,
    ...
    expect(findings).toContainEqual(
      expect.objectContaining({
        ...
        message: expect.stringContaining(
          "Revoked protection for unreferenced contract: DeadController",
        ),
      }),
    );
