...
describe("OptiPrune regressions", () => {
  it("does not report members from a dynamically imported module", async () => {
    ...
    const result = await analyze({ rootDir: root, entry: ["src/index.ts"], includeConventionalEntries: false, skip3: true, skip4: true });
  ...
  it("resolves pnpm store binaries to their actual packages", async () => {
    const root = await fixture({
      "package.json": JSON.stringify({ name: "pnpm-regression", scripts: { build: "tsc", test: "vitest" }, devDependencies: { typescript: "^5.8.3", vitest: "^3.2.7" } }),
    });
    const typescriptBin = path.join(root, "node_modules/.pnpm/typescript@5.8.3/node_modules/typescript/bin/tsc");
    const vitestBin = path.join(root, "node_modules/.pnpm/vitest@3.2.7/node_modules/vitest/vitest.mjs");
    ...
    const result = await analyze({ rootDir: root, includeConventionalEntries: false, reportUnusedExports: false });
    expect(result.findings.some((finding) => finding.evidence?.package === ".pnpm")).toBe(false);
    expect(result.findings.some((finding) => finding.rule === "missing-dev-dependency")).toBe(false);
  });

  it("classifies missing script tools as missing devDependencies", async () => {
    const root = await fixture({
      "package.json": JSON.stringify({ name: "missing-dev-regression", scripts: { build: "tsc" } }),
    });
    const result = await analyze({ rootDir: root, includeConventionalEntries: false, reportUnusedExports: false });
    expect(result.findings).toContainEqual(expect.objectContaining({
      rule: "missing-dev-dependency",
      evidence: expect.objectContaining({ package: "typescript", type: "devDependency" }),
    }));
  });
});
