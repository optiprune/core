...
describe("Plugin Integration Tests", () => {
  beforeAll(async () => {
    ...
    await fs.writeFile(
      path.join(testDir, "src", "commands", "hello.ts"),
      `
  ...
  it("should identify unused exports while respecting plugin-based usage", async () => {
    ...
    const unusedExport = report.findings.find(
      (f) => f.rule === "unused-export" && f.evidence.exportName === "unused",
    );
    const schemaExport = report.findings.find(
      (f) => f.rule === "unused-export" && f.evidence.exportName === "schema",
    );
  ...
  it("does not report exports from Oclif command modules as unused", async () => {
    ...
    const commandExport = report.findings.find(
      (finding) =>
        finding.rule === "unused-export" && finding.file.endsWith("/src/commands/hello.ts"),
    );
