...
test("Credit FAST elements registered via @customElement or static define()", async () => {
  ...
  assert.equal(
    issues.exports["imported-non-fast-element.ts"].ImportedNonFastElement.symbol,
    "ImportedNonFastElement",
  );
  // a FASTElement subclass that is never registered,
  assert.equal(
    issues.exports["unregistered-element.ts"].UnregisteredElement.symbol,
    "UnregisteredElement",
  );
