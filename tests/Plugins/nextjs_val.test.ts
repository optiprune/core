...
describe("Next.js App Router Validation", () => {
  it("should correctly identify entry points and unused components", async () => {
    ...
    const isUnusedFound = report.findings.some(
      (f) =>
        (f.rule === "unreachable-file" || f.rule === "unused-export") &&
        f.file.includes("UnusedComponent.tsx"),
    );
    const isButtonFound = report.findings.some(
      (f) => f.rule === "unused-export" && f.file.includes("Button.tsx"),
    );
