...
function makeContext(rootDir: string, modules: Map<string, ModuleRecord>): AnalysisContext {
  const options: ResolvedOptions = {
    rootDir, entry: [], extensions: [".ts", ".tsx", ".js", ".svelte"], ignore: [],
    reportUnusedExports: true, schemaEnums: {}, failOn: "none", json: false,
    includeConventionalEntries: false, pathAliases: new Map(), externalContracts: [],
    layers: { smtTimeoutMs: 1000, isolateMemoryLimitMb: 128, enableConcolicProof: false }, rules: {},
  };
  return { options, modules, entryPoints: new Set(), reachable: new Set(), maybeReachable: new Set(),
    components: [], usedExports: new Set(), usedPackages: new Set(), runtimeUsedFiles: new Set(),
    usedExportConfidence: new Map(), candidateBranches: [], };
}
...
describe("false-positive fixes", () => {
  it("resolves Angular templateUrl relative to the component", async () => {
    ...
    try {
      ...
      const modules = new Map([[sourceFile, parseModule("import { Component } from '@angular/core';\n@Component({ templateUrl: './foo.component.html' })\nexport class Foo {}", sourceFile)]]);
      ...
      engine.register(AngularPlugin); AngularPlugin.enabled = true;
  ...
  it("preserves the full Svelte scoped package name", async () => {
    ...
    try {
      ...
      const ctx = makeContext(root, new Map([[file, parseModule("import { page } from '@sveltejs/kit';", file)]]));
      const engine = new PluginEngine(); engine.register(SveltePlugin); SveltePlugin.enabled = true;
      await engine.run(ctx, { skipDetection: true });
  ...
  it("does not mark SvelteKit used without manifest evidence", async () => {
    ...
    try {
      ...
      await fs.mkdir(path.dirname(file), { recursive: true });
      const ctx = makeContext(root, new Map([[file, parseModule("export const prerender = true;", file)]]));
      const engine = new PluginEngine(); engine.register(SveltePlugin); SveltePlugin.enabled = true;
      await engine.run(ctx, { skipDetection: true });
