...
test("identifies conflict contention", async () => {
  ...
  const expected = [
    join(cwd, "overload-1.ts"),
    join(cwd, "overload-2.ts"),
    join(cwd, "overload-3.ts"),
  ];
...
test("detects circular dependency c → a → b → c", async () => {
  ...
  const hasCycle = file.cycles.some(
    (cycle) =>
      cycle.length === canonicalCycle.length &&
      cycle.every((value, idx) => value === canonicalCycle[idx]),
  );
  assert.ok(hasCycle);
});
...
test("false disables contention reporting", async () => {
  ...
  const withContention = session.describeFile(join(options.rootDir, "diamond-top.ts"));
  ...
  const filePath = join(options.rootDir, "diamond-top.ts");

  const withoutContention = buildFileDescriptor(filePath, options.rootDir, graph, entryPaths, {
