...
test("describes exports for a.ts", async () => {
  ...
  const hasCycle = file.cycles.some(
    (cycle) =>
      cycle.length === expectedCycle.length &&
      cycle.every((value, idx) => value === expectedCycle[idx]),
  );
  assert.ok(hasCycle);
...
test("tracks usage of default exports", async () => {
  ...
  assert.ok(
    defaultExport.importLocations.some((location) => location.filePath === join(cwd, "index.ts")),
  );
});
...
test("ignores namespace importers that never reference a member", async () => {
  ...
  assert.ok(
    roseExport.importLocations.every((location) => location.filePath !== namespaceImportPath),
  );
});
